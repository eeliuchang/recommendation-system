...
def cosine_correlation(v1, v2):
	v1_std = math.sqrt(sum([(v1[i]) ** 2 for i in range(len(v1))])/len(v1))
	v2_std = math.sqrt(sum([(v2[i]) ** 2 for i in range(len(v2))])/len(v2))
	cov = sum([(v1[i]) * (v2[i]) for i in range(len(v1))])/len(v1)
	return cov/v1_std/v2_std 



pref = {1:{'python':1, 'java':2, 'c':3}, 2: {'python':3, 'scala':2, 'java':1}}
sol1 = distance(pref, 1,2, cosine_correlation)
print(sol1)